...
from llavamod.model.multimodal_projector.pool_block import Pool_Block
from llavamod.model.multimodal_projector.qformer import qformer_config_template, Blip2Model, \
    cheap_qformer_config_template, \
    Cheap_Blip2Model
from llavamod.model.multimodal_projector.simple_block import SimpleBlock, Cheap_SimpleBlock
...
def build_image_projector(config, delay_load=False, **kwargs):
    ...
    if projector_type == 'linear':
        ...
        qformer_config = cheap_qformer_config_template(config, projector_type) if is_cheap else qformer_config_template(
        ...
        return Cheap_SimpleBlock(config.mm_hidden_size, config.hidden_size, num_in_block,
                                 num_out_block) if is_cheap else SimpleBlock(config.mm_hidden_size, config.hidden_size,
                                                                             num_in_block, num_out_block)
...
class build_projector(nn.Module):
    def __init__(self, config, delay_load=False, **kwargs):
        ...
        self.image_spatial_proj = build_image_projector(config, delay_load=False,
                                                        **kwargs) if mm_image_tower is not None else None

        if mm_video_tower is not None:
            self.video_patch_proj = build_video_projector(config, delay_load=False, **kwargs)
            self.video_spatial_proj = MLP(config.mm_hidden_size,
                                          config.hidden_size) if config.video_spatial_proj else None
            self.video_temproal_proj = MLP(config.mm_hidden_size,
                                           config.hidden_size) if config.video_temproal_proj else None
            self.video_global_proj = MLP(config.mm_hidden_size,
                                         config.hidden_size) if config.video_global_proj else None
    ...
    def forward_video(self, video_feature):
        ...
        patch_feature = self.video_patch_proj(
            rearrange(origin_patch_feature, 'b t n c -> (b t) n c'))  # [b, t, n, c] -> [bt, new_n, c]
        patch_feature = rearrange(patch_feature, '(b t) new_n c -> b t new_n c',
                                  b=b)  # [bt, new_n, c] -> [b, t, new_n, c]

        video_hidden_state = patch_feature

        if self.video_temproal_proj:
            temproal_feature = self.video_temproal_proj(origin_patch_feature.mean(2)).unsqueeze(
                2)  # [b, t, n, c] -> [b, t, 1, c]
