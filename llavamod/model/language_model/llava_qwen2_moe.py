...
class LLaVAMoDQwen2Config(Qwen2Config):
    ...
    def __init__(self,
        ...
        super(LLaVAMoDQwen2Config, self).__init__(**kwargs)


class LLaVAMoDQwen2Model(LlavaMetaModel, Qwen2Model):
    config_class = LLaVAMoDQwen2Config

    def __init__(self, config: Qwen2Config):
        super(LLaVAMoDQwen2Model, self).__init__(config)
...
def MoEQwen2Model_forward(self):
    def forward(
            # self,
...
class LLaVAMoDQwen2ForCausalLM(Qwen2ForCausalLM, LlavaMetaForCausalLM):
    config_class = LLaVAMoDQwen2Config

    def __init__(self, config):
        super(Qwen2ForCausalLM, self).__init__(config)
        self.model = LLaVAMoDQwen2Model(config)
    ...
    def initialize_moe_modules(self, model_args):
        ...
        rank0_print(f'replace Qwen2Model.forward to MoEQwen2Model.forward')
        # ipdb.set_trace()


class LLaVAMoDQwen2ForCausalLMFineTune(LLaVAMoDQwen2ForCausalLM):
    config_class = LLaVAMoDQwen2Config

    def __init__(self, config):
        super(LLaVAMoDQwen2ForCausalLMFineTune, self).__init__(config)
        # print(config)
...
class EvalLLaVAMoDQwen2ForCausalLM(LLaVAMoDQwen2ForCausalLM):
    config_class = LLaVAMoDQwen2Config

    def __init__(self, config):
        # print(config)
        super(EvalLLaVAMoDQwen2ForCausalLM, self).__init__(config)
        if getattr(self.config, 'lora', False) and self.config.lora.get('lora_enable', False):
            from peft import LoraConfig, get_peft_model
            pre_lora_config = self.config.lora
            lora_config = LoraConfig(
...
AutoConfig.register("moe_llava_qwen2", LLaVAMoDQwen2Config)
AutoModelForCausalLM.register(LLaVAMoDQwen2Config, LLaVAMoDQwen2ForCausalLM)
AutoModelForCausalLM.register(LLaVAMoDQwen2Config, LLaVAMoDQwen2ForCausalLMFineTune)
AutoModelForCausalLM.register(LLaVAMoDQwen2Config, EvalLLaVAMoDQwen2ForCausalLM)
