...
from llavamod.config.args import ModelArguments, DataArguments, TrainingArguments, DPOArguments
from llavamod.data.dataset import *
from llavamod.model import *
from llavamod.train.train_utils import *
from llavamod.train.dpo_trainer import DPOTrainer
...
def create_model_tokenizer(
        compute_dtype,
        dpo_model_type,
        dpo_model_name_or_path,
        model_args,
        training_args,
        data_args,
        bnb_model_from_pretrained_args):
    if dpo_model_type == "sparse":
        model_args.moe_enable = True
    else:
        model_args.moe_enable = False

    if dpo_model_name_or_path is not None:
        model_args.model_name_or_path = dpo_model_name_or_path
...
def train():
    ...
    # =================== dpo config =========================
    ref_model_type = dpo_args.ref_model_type
    policy_model_type = dpo_args.policy_model_type
    ref_model_name_or_path = dpo_args.ref_model_name_or_path
    policy_model_name_or_path = dpo_args.policy_model_name_or_path

    loss_type = dpo_args.loss_type
    moe_loss_enable = dpo_args.moe_loss_enable
    ...
    (
        ...
        policy_model_args
                               ...
                               dpo_model_type=policy_model_type,
                               dpo_model_name_or_path=policy_model_name_or_path,
    ...
    ref_model, *_ = create_model_tokenizer(compute_dtype=compute_dtype,
                                           dpo_model_type=ref_model_type,
                                           dpo_model_name_or_path=ref_model_name_or_path,
                                           model_args=copy.deepcopy(model_args),
                                           data_args=copy.deepcopy(data_args),
                                           training_args=copy.deepcopy(training_args),
                                           bnb_model_from_pretrained_args=copy.deepcopy(bnb_model_from_pretrained_args))
    ...
    # policy_training_args, policy_data_args, policy_model_args
    data_module = make_dpo_data_module(tokenizer=policy_tokenizer,
                                       data_args=policy_data_args)

    trainer = DPOTrainer(model=policy_model,
                         ref_model=ref_model,
