...
from llavamod.config.args import ModelArguments, DataArguments, TrainingArguments, AlignArguments
from llavamod.data.dataset import *
from llavamod.model import *
from llavamod.train.train_utils import *
from llavamod.train.align_trainer import AlignTrainer
...
def create_model_tokenizer(
        compute_dtype,
        align_model_type,
        align_model_name_or_path,
        align_model_pretrain_mm_mlp_adapter,
        model_args,
        training_args,
        data_args,
        bnb_model_from_pretrained_args):
    if align_model_type == "sparse":
        model_args.moe_enable = True
    else:
        model_args.moe_enable = False

    if align_model_name_or_path is not None:
        model_args.model_name_or_path = align_model_name_or_path
    rank0_print("##### model name: ", model_args.model_name_or_path)

    if align_model_pretrain_mm_mlp_adapter is not None:
        model_args.pretrain_mm_mlp_adapter = align_model_pretrain_mm_mlp_adapter
    rank0_print("##### mm mlp adaptor name: ", model_args.pretrain_mm_mlp_adapter)
    ...
    if model_args.image_tower is not None or model_args.video_tower is not None:
        if not model_args.moe_enable:
            ...
            raise ValueError("model type is confusing")
    else:
    ...
    return model, tokenizer, training_args, data_args, model_args
...
def train():
    ...
    parser = transformers.HfArgumentParser(
        (ModelArguments, DataArguments, TrainingArguments, AlignArguments))
    model_args, data_args, training_args, align_args = parser.parse_args_into_dataclasses()
    local_rank = training_args.local_rank
    compute_dtype = (torch.float16 if training_args.fp16 else (torch.bfloat16 if training_args.bf16 else torch.float32))

    # =================== align config =========================
    ref_model_type = align_args.ref_model_type
    policy_model_type = align_args.policy_model_type
    ref_model_name_or_path = align_args.ref_model_name_or_path
    policy_model_name_or_path = align_args.policy_model_name_or_path
    policy_model_pretrain_mm_mlp_adapter = align_args.policy_pretrain_mm_mlp_adapter
    ref_model_pretrain_mm_mlp_adapter = align_args.ref_pretrain_mm_mlp_adapter

    loss_type = align_args.loss_type
    moe_loss_enable = align_args.moe_loss_enable
    ...
    (
        ...
        policy_data_args,
        policy_model_args
    ) = create_model_tokenizer(compute_dtype=compute_dtype,
                               align_model_type=policy_model_type,
                               align_model_name_or_path=policy_model_name_or_path,
                               align_model_pretrain_mm_mlp_adapter=policy_model_pretrain_mm_mlp_adapter,
                               model_args=copy.deepcopy(model_args),
                               data_args=copy.deepcopy(data_args),
                               training_args=copy.deepcopy(training_args),
                               bnb_model_from_pretrained_args=copy.deepcopy(bnb_model_from_pretrained_args))

    # rank0_print("########### before training args, ", policy_training_args)
    # rank0_print("########### before model args, ", policy_model_args)

    ref_model, *_ = create_model_tokenizer(compute_dtype=compute_dtype,
                                           align_model_type=ref_model_type,
                                           align_model_name_or_path=ref_model_name_or_path,
                                           align_model_pretrain_mm_mlp_adapter=ref_model_pretrain_mm_mlp_adapter,
                                           model_args=copy.deepcopy(model_args),
                                           data_args=copy.deepcopy(data_args),
                                           training_args=copy.deepcopy(training_args),
                                           bnb_model_from_pretrained_args=copy.deepcopy(bnb_model_from_pretrained_args))

    rank0_print("########### training args, ", policy_training_args)
    rank0_print("########### model args, ", policy_model_args)

    # import pdb;
    # pdb.set_trace()

    rank0_print("#### ref model exists: ", ref_model is not None)

    data_module = make_supervised_data_module(tokenizer=policy_tokenizer,
                                              data_args=policy_data_args)

    trainer = AlignTrainer(model=policy_model,
                           ref_model=ref_model,
                           args=policy_training_args,
                           tokenizer=policy_tokenizer,
                           loss_type=loss_type,
                           moe_loss_enable=moe_loss_enable,
                           **data_module)

    if list(pathlib.Path(policy_training_args.output_dir).glob("checkpoint-*")):
        trainer.train(resume_from_checkpoint=True)
    else:
        trainer.train()
    trainer.save_state()

    # policy_tokenizer.config.use_cache = True

    if policy_training_args.lora_enable and not policy_model_args.moe_enable:
        state_dict = get_peft_state_maybe_zero_3(
            policy_model.named_parameters(), policy_training_args.lora_bias
        )
        non_lora_state_dict = get_peft_state_non_lora_maybe_zero_3(
            policy_model.named_parameters()
        )
        if policy_training_args.local_rank == 0 or policy_training_args.local_rank == -1:
            policy_model.config.save_pretrained(policy_training_args.output_dir)
            policy_model.save_pretrained(policy_training_args.output_dir, state_dict=state_dict)
            torch.save(non_lora_state_dict, os.path.join(policy_training_args.output_dir, 'non_lora_trainables.bin'))
    else:
        safe_save_model_for_hf_trainer(trainer=trainer,
                                       output_dir=policy_training_args.output_dir)
        if policy_model_args.moe_enable:
            ckpt = policy_model.state_dict()
            ...
            torch.save(ckpt, os.path.join(policy_training_args.output_dir, 'pytorch_model.bin'))
            policy_model.config.save_pretrained(policy_training_args.output_dir)
            if policy_training_args.local_rank == 0 or policy_training_args.local_rank == -1:
                [os.remove(i) for i in glob(os.path.join(policy_training_args.output_dir, 'adapter_*'))]
